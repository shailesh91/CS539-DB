...
public class ChessGameB extends Configured implements Tool {
		...
		public void map(final LongWritable key, final Text value,
			...
			try {
				...
				if (!game.toString().contains("null")) {
					...
					if (R == 2) {
						context.write(new Text(B), new Text("B-W-1"));
						context.write(new Text(W), new Text("W-L-1"));
					} else if (R == 1) {
						context.write(new Text(B), new Text("B-D-1"));
						context.write(new Text(W), new Text("W-D-1"));
					} else if (R == 0) {
						context.write(new Text(B), new Text("B-L-1"));
						context.write(new Text(W), new Text("W-W-1"));
					}
				}
			} catch (Exception ee) {
	...
	/** Combiner. */
	public static class PlayerWLDCombiner extends Reducer<Text, Text, Text, Text>{
		/** Implement reduce(). */
		@Override
		public void reduce(final Text key, final Iterable<Text> values,
				final Context context) throws IOException, InterruptedException {
			int bw = 0, bl = 0, bd = 0, ww = 0, wl = 0, wd = 0;
			for (Text val : values) {
			...
			if(bw !=0) context.write(key, new Text("B-W-"+bw));
			if(bl !=0) context.write(key, new Text("B-L-"+bl));
			if(bd !=0) context.write(key, new Text("B-D-"+bd));
			if(ww !=0) context.write(key, new Text("W-W-"+ww));
			if(wl !=0) context.write(key, new Text("W-L-"+wl));
			if(wd !=0) context.write(key, new Text("W-D-"+wd));
		}
	}
	...
	public int run(final String[] args) throws Exception {
		...
		job.setMapperClass(PlayerWLDMapper.class);
		job.setCombinerClass(PlayerWLDCombiner.class);
		job.setReducerClass(PlayerWLDReducer.class);
