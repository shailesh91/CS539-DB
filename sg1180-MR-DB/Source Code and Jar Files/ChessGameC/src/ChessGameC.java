...
public class ChessGameC extends Configured implements Tool {
	...
	public static class ChessMovesMapper extends
	...
	/** Reducer. */
	public static class ChessMovesSumReducer extends
	...
	public static class ChessMovesPercentageMapper extends
			Mapper<Text, Text, Text, Text> {
		// ** mapper(). */
		@Override
		public void map(final Text key, final Text value, final Context context)
				throws IOException, InterruptedException {
			context.write(new Text("1-" + value.toString()),
					new Text(key.toString()));
		}
	}

	public static class ChessMovesPercentageReducer extends
		...
		public void reduce(final Text key, final Iterable<Text> values,
			...
			for (String q : qe) {
				String[] aq = q.split("-");
				float percentage = (float) Integer.parseInt(aq[0]) / sum;
				percentage *= 100;
				context.write(new Text(aq[1]), new Text(percentage + "%"));
			}
		}
	}
	...
	public int run(final String[] args) throws Exception {
		...
		job.setMapperClass(ChessMovesMapper.class);
		job.setCombinerClass(ChessMovesSumReducer.class);
		job.setReducerClass(ChessMovesSumReducer.class);
		...
		final Job job2 = Job.getInstance(conf, "ChessGameC: " + input + "->"
				+ output);

		job2.setJarByClass(ChessGameC.class);

		job2.setMapperClass(ChessMovesPercentageMapper.class);
		job2.setReducerClass(ChessMovesPercentageReducer.class);
